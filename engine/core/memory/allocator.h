...
struct HeapAllocator {
  ...
  struct Allocation {
    ...
    FORCE_INLINE constexpr size_t
        calc_required_capacity_size(size_t fit_size,
                                    size_t capacity_interval) const {
      size_t interval_count =
          (size_t)fiwre::round_up((double)fit_size / capacity_interval);
      if (interval_count > 0) {
...
struct FixedAllocation {
  ...
  struct Allocation {
    ...
    FORCE_INLINE constexpr const T& operator[](size_t index) const {
      return ptr[index];
    }
    ...
    FORCE_INLINE constexpr size_t
      ...
      size_t interval_count =
          (size_t)fiwre::round_up((double)fit_size / cap_interval_size);
