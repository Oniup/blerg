...
struct BasicString {
  ...
  constexpr BasicString(const StringView& view)
    ...
    fiwre::copy_items(begin(), end(), view.data());
    _allocator[_size] = '\0';
  }
  ...
  constexpr BasicString(const BasicString& str)
    ...
    fiwre::copy_items(begin(), end(), str._allocator.ptr);
    _allocator[_size] = '\0';
  }

  constexpr BasicString(BasicString&& str)
    : _size(fiwre::move(str._size)), _allocator(fiwre::move(str._allocator)) {
    str._size = 0;
  ...
  // constexpr bool is_empty() const { return _allocator.is_empty() || (_size ==
  // 0 || _size == NoPos);
  // }
  ...
  constexpr BasicString& operator=(BasicString&& str) {
    _size      = fiwre::move(str._size);
    _allocator = fiwre::move(str._allocator);
  ...
  constexpr bool operator==(const char* cstr) const {
    return StringViewCast<BasicString>::Cast(*this).Compare(
        StringView(cstr, fiwre::cstr_length(cstr)));
  }
  ...
  constexpr const StringView slice(ConstIterator begin,
  ...
  constexpr BasicString copy_slice(ConstIterator begin,
                                   ConstIterator end) const {
    return BasicString(slice(begin.ptr(), end.ptr() - begin.ptr()));
  }

  constexpr void write_to_buffer(char* buffer, size_t maxSize) const {
    std::strncpy(buffer, _allocator.ptr,
                 fiwre::clamp<size_t>(_size, 0, maxSize));
  }
  ...
  constexpr void to_pascal_case() {
    ...
    for (size_t i = 0; i < _size; i++) {
      if (first) {
        _allocator[i] = (char)toupper(_allocator[i]);
        first         = false;
      } else if (_allocator[i] == ' ' || _allocator[i] == '-' ||
                 _allocator[i] == '_') {
        Iterator offset = begin() + i;
        erase(offset, offset + 1);
        _allocator[i] = (char)toupper(_allocator[i]);
  ...
  constexpr size_t find_last(const char* str) const {
    return find_last(StringView(str));
  }

  template <typename _ArrayAllocator>
  constexpr void FindAll(Array<size_t, _ArrayAllocator>& buffer,
  ...
  constexpr void replace(const BasicString& search,
                         const BasicString& replace) {
    this->replace(StringView(search._allocator.ptr, search._size),
                  StringView(replace._allocator.ptr, replace._size));
  }

  constexpr bool replace_first(const BasicString& search,
                               const BasicString& replace) {
    return replace_first(StringView(search._allocator.ptr, search._size),
                         StringView(replace._allocator.ptr, replace._size));
  }

  constexpr bool replace_last(const BasicString& search,
                              const BasicString& replace) {
  ...
  constexpr bool replace_first(const StringView& search,
                               const StringView& replace) {
    size_t offset = find(search);
    return _replace_instance(offset, search.size(), replace);
  }

  constexpr bool replace_last(const StringView& search,
                              const StringView& replace) {
...
private:
  ...
  constexpr bool _replace_instance(size_t pos, size_t search_size,
                                   const StringView& replace) {
    if (pos == NoPos) {
      return false;
    }
...
struct StringViewCast<
    BasicString<T, _Allocator, _CapacityIncreaseIntervalSize>> {
  static constexpr StringView Cast(
      const BasicString<T, _Allocator, _CapacityIncreaseIntervalSize>& str) {
